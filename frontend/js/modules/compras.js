  ...
  try {
    const [compras, proveedores] = await Promise.all([
      ...
      state.proveedores.length
        ? Promise.resolve(state.proveedores)
        : apiGet('proveedores') || []
    ]);
...
async function guardarCompra() {
  ...
  if (!compraItems.length) { alert('Agrega al menos un producto'); return; }
  ...
  try {
    ...
    const categoriasUnicas = [...new Set(
      compraItems.map(x => x.categoria).filter(c => c)
    )];
    ...
    console.error(e);
    alert('Error al sincronizar con Sheets');
  }
}